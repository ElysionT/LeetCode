package org.demo.string;

import java.util.Arrays;

/**
 * 移除元素
 * https://leetcode.cn/leetbook/read/array-and-string/cwuyj/
 * 
 * 给你一个数组 nums 和一个值 val，你需要 原地 移除所有数值等于 val 的元素，并返回移除后数组的新长度。
 * 不要使用额外的数组空间，你必须仅使用 O(1) 额外空间并 原地 修改输入数组。
 * 元素的顺序可以改变。你不需要考虑数组中超出新长度后面的元素。
 * 
 * 示例 1：
 * 输入：nums = [3,2,2,3], val = 3
 * 输出：2, nums = [2,2]
 * 解释：函数应该返回新的长度 2, 并且 nums 中的前两个元素均为 2。你不需要考虑数组中超出新长度后面的元素。
 * 例如，函数返回的新长度为 2 ，而 nums = [2,2,3,3] 或 nums = [2,2,0,0]，也会被视作正确答案。
 * 
 * 示例 2：
 * 输入：nums = [0,1,2,2,3,0,4,2], val = 2
 * 输出：5, nums = [0,1,4,0,3]
 * 解释：函数应该返回新的长度 5, 并且 nums 中的前五个元素为 0, 1, 3, 0, 4。注意这五个元素可为任意顺序。你不需要考虑数组中超出新长度后面的元素。
 * 
 * 提示：
 * 0 <= nums.length <= 100
 * 0 <= nums[i] <= 50
 * 0 <= val <= 100
 */
public class RemoveElement {

	/*
	 * 执行用时：0 ms, 在所有 Java 提交中击败了100.00%的用户
	 * 内存消耗：40.1 MB, 在所有 Java 提交中击败了48.19%的用户
	 * 通过测试用例：113 / 113
	 */
	public int removeElement(int[] nums, int val) {
		int result = 0;
		int iVal;
		for (int i = 0, length = nums.length; i < length; i++) {
			iVal = nums[i];
			if (iVal != val) {
				if (i != result) {
					nums[result] = nums[i];
					nums[i] = 0;
				}
				result++;
			}
		}
		return result;
	}

	public static void main(String[] args) {
		// 2
		int[] nums1 = { 3, 2, 2, 3 };
		int val1 = 3;
		// 5
		int[] nums2 = { 0, 1, 2, 2, 3, 0, 4, 2 };
		int val2 = 2;

		long start;
		int result;
		RemoveElement solution = new RemoveElement();

		System.out.println("nums1:" + Arrays.toString(nums1) + ", val1:" + val1);
		start = System.currentTimeMillis();
		result = solution.removeElement(nums1, val1);
		System.out.println("Time:" + (System.currentTimeMillis() - start) + ", 2 - Result:" + result + "\n");

		System.out.println("nums2:" + Arrays.toString(nums2) + ", val2:" + val2);
		start = System.currentTimeMillis();
		result = solution.removeElement(nums2, val2);
		System.out.println("Time:" + (System.currentTimeMillis() - start) + ", 5 - Result:" + result + "\n");
	}

}
